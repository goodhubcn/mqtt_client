...
#endif
{
	...
	else
	{
		if(*sock <= 0)
		{
			Log(TRACE_MIN, -1, "1138 New socket %d for %s, port %d",	*sock, addr, port);
			*sock =	socket(family, type, 0);
			Log(TRACE_MIN, -1, "1140 New socket %d for %s, port %d",	*sock, addr, port);
		}
...
*/
			...
			else
			{
				...
				Log(TRACE_MIN, -1, "New socket %d for %s, port %d",	*sock, ip_str, port);
				// unsigned int mark_netid = 0xfff8; // 绑定私网
				// int vRet = setsockopt(*sock, SOL_SOCKET, SO_MARK, &mark_netid, sizeof(mark_netid));
				int mark;
				socklen_t len = sizeof(mark);
				if (getsockopt(*sock, SOL_SOCKET, SO_MARK, &mark, &len) == 0) {
					Log(TRACE_MIN, -1, "New socket %d for %s, port %d mark:%x",	*sock, ip_str, port,mark);
				}
				if (family == AF_INET)
