	...
	*/
	...
	if (command->command.type == CONNECT)
	{
		...
		else
		{
			...
			if(command->client->connectPre)
			{
				if(command->client->c->net.socket <= 0)
				{
					command->client->c->net.socket = socket(AF_INET,SOCK_STREAM,0);
				}
				command->client->connectPre(command->client->connectPre_context,command->client->c->net.socket);
			}
