...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
 ...
 #if  MEM_CERTIFICATE
      m->c->sslopts->mem_root_cer    = options->ssl->mem_root_cer;
	  m->c->sslopts->mem_client_cer  = options->ssl->mem_client_cer;
	  m->c->sslopts->mem_key_cer     = options->ssl->mem_key_cer;
 #endif
...
int MQTTAsync_setConnectPrepare(MQTTAsync handle, void* context, MQTTAsync_connect_prepare* prepare)
{
	int rc = MQTTASYNC_SUCCESS;
	MQTTAsyncs* m = handle;
	...
	{
		m->connectPre_context = context;
		m->connectPre = prepare;
	}
