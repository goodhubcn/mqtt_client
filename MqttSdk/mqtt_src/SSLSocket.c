...
int SSL_CTX_use_certificate_chain(SSL_CTX *ctx, const char *mem_str)
{
    BIO *in;
    int ret = 0;
    X509 *x = NULL;
    ...
    in = BIO_new_mem_buf(mem_str, strlen(mem_str));
    if (in == NULL) {
...
*/
	...
	{
#if MEM_CERTIFICATE
        //printf("liping add mem_client_cer:\r\n%s",opts->mem_client_cer);
        if ((rc = SSL_CTX_use_certificate_chain(net->ctx, opts->mem_client_cer)) != 1)
		...
		if (opts->privateKeyPassword != NULL)
		{
			...
			SSL_CTX_set_default_passwd_cb_userdata(net->ctx, (void*)opts->privateKeyPassword);
               ...
               // printf("liping add mem_key_cer:\r\n%s",opts->mem_key_cer);

	            BIO * cbio = BIO_new_mem_buf( opts->mem_key_cer, strlen(opts->mem_key_cer));
                EVP_PKEY *pkey = NULL;
...
#if MEM_CERTIFICATE
            //printf("liping add mem_root_cer:\r\n%s",opts->mem_root_cer);
	         BIO * cbio = BIO_new_mem_buf(opts->mem_root_cer, strlen(opts->mem_root_cer));
...
int SSLSocket_getch(SSL* ssl, SOCKET socket, char* c)
{
	...
	{
		 rc = 1;
		 Log(TRACE_PROTOCOL, -1, "liping err set  rc = 1");
...
char *SSLSocket_getdata(SSL* ssl, SOCKET socket, size_t bytes, size_t* actual_len, int* rc)
{
	...
	if (*actual_len != bytes)
	{
		...
		*rc = SSL_read(ssl, buf + (*actual_len), (int)(bytes - (*actual_len)));
